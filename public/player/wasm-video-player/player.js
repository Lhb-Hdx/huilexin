var bni = 0;
// setTimeout(function(){bni=0},20000)
//Decoder states.
...
Player.prototype.initDownloadWorker = function () {
    var self = this;
    this.downloadWorker = new Worker("./wasm-video-player/downloader.js");
    this.downloadWorker.onmessage = function (evt) {
        var objData = evt.data;
        // console.log(objData.t);
        switch (objData.t) {
...
Player.prototype.initDecodeWorker = function () {
    var self = this;
    this.decodeWorker = new Worker("./wasm-video-player/decoder.js");
...
Player.prototype.displayVideoFrame = function (frame) {
    ...
    if (audioTimestamp <= 0 || delay <= 0) {
        ...
        // if(bni < (25*5)){
        //     download(data,"gl"+Date.now()+".yuv","image/yuv");
        //     bni++
        // }
...
Player.prototype.startTrackTimer = function () {
  // 注释原因：改为在组件调用 self.player.updateTrackTime() 返回播放进度，这里的定时器获取就取消掉
  /* var self = this;
...
Player.prototype.updateTrackTime = function () {
    if (this.playerState == playerStatePlaying && this.pcmPlayer) {
        ...
        if(this.ontimeupdate){
            this.ontimeupdate(currentPlayTime)
        }
        if (this.timeTrack) {
            this.timeTrack.value = 1000 * currentPlayTime;
        }
        if(this.formatTime(currentPlayTime) == this.displayDuration){
            this.stopTrackTimer();
            if(this.onstop){
                this.onstop()
            }
        ...
        return [this.formatTime(currentPlayTime), this.formatTime(this.duration / 1000)];
    }
};
Player.prototype.getCurrentTime = function(){
    return this.pcmPlayer.getTimestamp() + this.beginTimeOffset;
}
