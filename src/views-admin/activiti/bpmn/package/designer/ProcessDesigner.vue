    ...
    <div class="my-process-designer__header">
      ...
      <template v-if="!$slots['control-header']">
        <el-button-group key="file-control">
          <el-button v-permission="['save']" :loading="loading" :size="headerButtonSize" icon="el-icon-folder-opened" @click="$refs.dialog.open()">保存</el-button>
...
// 提交弹窗
import SubmitDialog from '@/views-admin/activiti/bpmn/components/submitDialog'
  ...
  methods: {
    ...
    async getOmProcessModel() {
      let { model, code } = await request({
        url: api.linkageManage.getOmProcessModel,
        data: { id: this.id }
      })
      if (code !== 1) return
      this.modelInfo = model
      this.createNewDiagram(model.svgXml)
    },
    initModelListeners() {
