<template>
  <div ref="myProcessDesigner" class="my-process-designer" :style="{ height: mapHeight + 'px' }">
    <div v-if="L" class="my-process-designer__header">
      <slot name="control-header" />
      <template v-if="!$slots['control-header']">
        <el-button-group key="file-control">
          <el-button :loading="loading" :size="headerButtonSize" icon="el-icon-folder-opened" @click="$refs.dialog.open()">保存</el-button>
        ...
        <el-button-group key="scale-control">
          ...
          <el-button :size="headerButtonSize" class="pb-8">{{ Math.floor(defaultZoom * 10 * 10) + '%' }}</el-button>
        ...
        <el-button-group key="stack-control">
          ...
          <el-button :size="headerButtonSize" @click="look">查看</el-button>
        </el-button-group>
      </template>
      <!-- 用r于打开本地文件-->
    ...
    <!-- 提交弹窗 -->
    <SubmitDialog ref="dialog" :bpmn-modeler="bpmnModeler" :model-info="modelInfo" />
  </div>
</template>
...
// 提交弹窗
import SubmitDialog from '@/views-admin/opm-maintenance/process-manage/bpmn/components/submitDialog'
...
export default {
  ...
  mounted() {
    ...
    this.mapHeight = window.innerHeight - this.$refs.myProcessDesigner.getBoundingClientRect().top
  },
  methods: {
    async look() {
    ...
    initBpmnModeler() {
      ...
      if (this.id) {
        this.getOmProcessModel()
      }
    },
    async getOmProcessModel() {
      const { model, code } = await request({
        url: api.activity.getNewOaActivity,
        data: { id: this.id }
      })
      if (code !== 1) return
      this.$store.dispatch('bpmn/setBpmnModelInfo', {
        type: model.type
      })
      this.modelInfo = model
      this.createNewDiagram(model.xmlData)
    },
    initModelListeners() {
