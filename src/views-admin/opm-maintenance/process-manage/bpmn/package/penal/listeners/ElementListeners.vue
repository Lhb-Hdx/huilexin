<template>
  <div class="panel-tab__content">
    ...
    <el-drawer :visible.sync="listenerFormModelVisible" title="执行监听器" :size="`${width}px`" append-to-body destroy-on-close>
      <el-form ref="listenerFormRef" size="mini" :model="listenerForm" label-width="96px" @submit.native.prevent>
        ...
        <el-form-item label="监听器类型" prop="listenerType" :rules="{ required: true, trigger: ['blur', 'change'] }">
          <el-select v-model="listenerForm.listenerType" @change="changeListen">
        ...
        >
          <el-input v-model="listenerForm.class" :readonly="true" />
        </el-form-item>
        <el-form-item
    ...
    <el-dialog title="字段配置" :visible.sync="listenerFieldFormModelVisible" width="600px" append-to-body destroy-on-close>
      <el-form ref="listenerFieldFormRef" :model="listenerFieldForm" size="mini" label-width="96px" @submit.native.prevent>
        ...
        <el-form-item v-if="listenerFieldForm.fieldType === 'string' && listenerFieldForm.name === 'type'" label="动作类型：" prop="string" :rules="{ required: true, trigger: ['blur', 'change'] }">
          <el-select v-model="listenerFieldForm.string">
            <el-option v-for="i in actionTypes" :key="i.value" :label="i.label" :value="i.value" />
          </el-select>
        </el-form-item>
        <el-form-item v-if="listenerFieldForm.fieldType === 'string' && listenerFieldForm.name === 'data'" label="用户：">
          <!--          <el-select v-model="selectActionUsers" v-loadmore="loadMoreUser" multiple>-->
          <!--            <el-option v-for="i in users" :key="i.userId" :label="i[findUserListApi === api.sys.findWinxinUser ? 'realname' : 'realName']" :value="i.userId" />-->
          <!--          </el-select>-->
          <el-button type="primary" @click="openShowUserListDialog">选择用户</el-button>
          <div>
            <el-tag
              v-for="item in currentSelectUser"
              :key="item.userId"
              size="mini"
              effect="dark"
...
export default {
  ...
  data() {
    return {
      ...
      actionTypes: [
        { value: '微信推送', label: '微信推送' },
        { value: '短信推送', label: '短信推送' },
        { value: '邮件推送', label: '邮件推送' },
        { value: '脚本执行', label: '脚本执行' },
        { value: '作业执行', label: '作业执行' }
      ],
      users: [],
      findSysUserQuery: {
        page: 1,
  ...
  methods: {
    ...
    openListenerForm(listener, index) {
      if (listener) {
        this.listenerForm = initListenerForm(this.$tool.deepCopy(listener))
    ...
    openListenerFieldForm(field, index) {
      this.users = [
        { userId: 'assignee', realName: '审核用户', realname: '审核用户' },
        { userId: 'approval', realName: '申请用户', realname: '申请用户' },
        { userId: 'createUserId', realName: '创建用户', realname: '创建用户' },
      ...
      if (this.listenerFieldForm.name === 'data') {
        const idx = this.fieldsListOfListener.findIndex(items => items.name === 'type' && (items.string === '微信推送' || items.string === '短信推送' || items.string === '邮件推送'))
        if (idx !== -1) {
          this.findUserListApi = this.api.sys.findWinxinUser
        } else {
          this.findUserListApi = this.api.sys.findSysUser
        }
      }
      ...
      if (this.listenerFieldForm.name === 'data' && this.listenerFieldForm.fieldType === 'string' && this.listenerFieldForm.string.length > 0) {
        this.selectActionUsers = JSON.parse(this.listenerFieldForm.string)
        // const selectActionUsers = []
        // userData.forEach(item => selectActionUsers.push(item.userId))
        // this.selectActionUsers = selectActionUsers
        const ids = []
        this.selectActionUsers.forEach(item => ids.push(item.userId))
        this.getUserList({ ids: ids.toString(), limit: -1 }, [])
      }
    },
    // 保存监听器注入字段
    async saveListenerFiled() {
      if (this.selectActionUsers.length > 0 && this.listenerFieldForm.name === 'data' && this.listenerFieldForm.fieldType === 'string') {
        // const userObject = []
        // this.selectActionUsers.forEach(item => {
        //   userObject.push({
        //     userId: item
        //   })
        // })
        this.$set(this.listenerFieldForm, 'string', JSON.stringify(this.selectActionUsers))
      }
    ...
    changeListen(val) {
      if (val === 'classListener') this.$set(this.listenerForm, 'class', 'com.hy.listener.ExecutionListenerService')
    },
    loadMoreUser() {
    ...
    async findSysUser() {
      let { data, code, model } = await this.$request({ url: this.findUserListApi, data: { ...this.findSysUserQuery, companyId: this.$auth.getUserSession().companyId }})
      if (code === 1) {
        if (this.findUserListApi === this.api.sys.findWinxinUser) {
          if (model) {
            this.users = [...this.users, ...JSON.parse(model)]
          }
        } else {
    ...
    openShowUserListDialog() {
      console.log(this.selectActionUsers)
      this.$refs.userListTable.open({
        isRadio: false,
        allSelection: this.selectActionUsers || [],
        api: this.findUserListApi,
        keyField: 'userId',
        initFindQuery: this.findUserListApi === this.api.sys.findWinxinUser ? { page: 1, limit: -1 } : { page: 1, limit: 10 },
        pageChangeFindDataEvType: this.findUserListApi === this.api.sys.findWinxinUser ? 'local' : 'request'
      })
    },
    submitSelect(val) {
      console.log('val: ', val)
      const selectActionUsers = []
      val.forEach(item => selectActionUsers.push({ userId: item.userId }))
      this.selectActionUsers = selectActionUsers
      const ids = []
      this.selectActionUsers.forEach(item => ids.push(item.userId))
      this.getUserList({ ids: ids.toString(), limit: -1 })
    ...
    async getUserList(query) {
      const { code, data, model } = await this.$request({
      ...
      if (code === 1) {
        if (this.findUserListApi === this.api.sys.findWinxinUser) {
          ...
          const fixedUserFilter = this.users.filter(el => userIds.indexOf(el.userId) !== -1)
          const modelData = JSON.parse(model)
          const idsArr = query.ids.split(',')
          this.currentSelectUser = [...modelData.filter(el => idsArr.indexOf(String(el.userId)) !== -1), ...fixedUserFilter]
        } else {
          this.currentSelectUser = data
        }
      }
    },
    handleClose(userId) {
      const currentSelectUserIndex = this.currentSelectUser.findIndex(item => +item.userId === +userId)
      const selectActionUsersIndex = this.selectActionUsers.findIndex(item => +item.userId === +userId)
      if (currentSelectUserIndex !== -1) {
        this.currentSelectUser.splice(currentSelectUserIndex, 1)
      }
      if (selectActionUsersIndex !== -1) {
        this.selectActionUsers.splice(selectActionUsersIndex, 1)
      }
    }
  }
}
