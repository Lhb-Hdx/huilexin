<template>
  <div>
    <ScriptTable ref="scriptTable" :router-name="{ create: 'AdminScriptLibraryCreate' }">
      <el-table-column label="操作" width="300" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-permission="['RunScript']" size="mini" type="primary" @click="onRunScript(row)">调试</el-button>
...
export default {
  ...
  methods: {
    ...
    // 编辑
    onEdit(row) {
      this.$tool.storagePageTableQuery('set', this.$route.name, this.$refs.scriptTable.listQuery)
