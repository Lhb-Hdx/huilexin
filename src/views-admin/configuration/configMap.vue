<template>
  <div class="config-map">
    <MapLeft
      :menu-list.sync="areaTree"
      @changeCenter="center=$event"
      @updateMenuList="findHyitArea"
    />
    <baidu-map
      v-if="show"
      :center="center"
      ...
      >
        <bm-context-menu :width="200">
          <bm-context-menu-item
            ...
            :key="child.id"
            :callback="onMarkerClick.bind(this,child)"
            :text="child.name"
          />
        </bm-context-menu>
        <bm-context-menu :width="200">
    ...
    <div v-if="!show" class="map-box flex-h-center mt-80" style="flexDirection: column">
      ...
      <ul class="ml-0">
        <li v-for="item in diagramlist" :key="item.id" class="font-16  ml-0">
          <span> {{ item.name }}</span>
          <a class="ml-4" @click="onMarkerClick(item)">[点击查看]</a>
        </li>
      </ul>
    </div>
    <MapDialog @updateMenuList="findHyitArea" />
  </div>
</template>
<script>
...
export default {
  ...
  components: {
    MapLeft,
    MapDialog: () => import('./component/mapDialog')
  },
  data() {
    return {
      center: { lng: 0, lat: 0 },
