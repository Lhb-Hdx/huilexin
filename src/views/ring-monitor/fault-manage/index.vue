    ...
    <div class="fault-table">
      <div class="filter-container">
        <div class="filter-container-left">
          ...
          <el-tag v-for="(item,index) in levelData" :key="index" class="filter-item" :class="item.isActive ? 'active' : ''" :color="judgmentColor(item.level) ? item.color : 'gray'" effect="dark" @click="handleCatLevelClick(item)">{{ item.name }}</el-tag>
      ...
      <div v-loading="loading" class="table-wrapper">
        ...
        >
          ...
          <el-table-column label="级别" align="center" :width="setWidth('级别')" show-overflow-tooltip>
            <template slot-scope="{row}">
              <el-tag effect="dark" :color="setLevelTagColor(row.alevel)">{{ setLevelTagName(row.alevel) }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="标题" align="left" :width="setWidth('标题')" show-overflow-tooltip>
            <template slot-scope="{row}">
          ...
          <el-table-column type="expand" prop="id" width="1">
            <template slot-scope="props">
              ...
              >
                <el-table-column label="级别" align="left" :width="setWidth('级别')" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <el-tag effect="dark" :color="setLevelTagColor(row.alevel)">{{ setLevelTagName(row.alevel) }}</el-tag>
                  </template>
                </el-table-column>
                ...
                <el-table-column label="设备名称" align="left" :width="setWidth('设备名称')" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <el-link type="primary" @click="handlerToItMonitorDevice(row)">{{ row.hyMarkName }}</el-link>
                  </template>
                </el-table-column>
                <el-table-column label="IP" align="left" :width="setWidth('IP')" prop="ip" show-overflow-tooltip />
                <el-table-column label="类型" align="left" :width="setWidth('类型')" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <span>{{ setLeaveType(row.devTypeId) }}</span>
                  </template>
                </el-table-column>

                <el-table-column label="故障时间" :width="setWidth('故障时间')" align="right" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <span>{{ setFormatMsgTime(row.clock) }}</span>

                  </template>
                </el-table-column>
                <el-table-column label="跟进时间" :width="setWidth('跟进时间')" align="right" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <span>{{ setRCTime(row.clock, row.rclock) }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="推送状态" :width="setWidth('推送状态')" align="left" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <span :style="{ color: setPushMsgColor(row.pushMsg) }">{{ setPushMsg(row.pushMsg) }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="处理人" :width="setWidth('处理人')" align="left" show-overflow-tooltip>
                  <template slot-scope="{row}">
                    <span>{{ setUser(row.userId) }}</span>
                  </template>
                </el-table-column>
                <el-table-column label="状态" align="left" :width="setWidth('状态')" show-overflow-tooltip>
                  <template slot-scope="{row}">
          ...
          <el-table-column label="设备名称" :width="setWidth('设备名称')" align="left" show-overflow-tooltip>
            <template slot-scope="{row}">
              <el-link type="primary" @click="handlerToItMonitorDevice(row)">{{ row.hyMarkName }}</el-link>
            </template>
          </el-table-column>
          ...
          <el-table-column label="状态" :width="setWidth('状态')" align="left" show-overflow-tooltip>
            <template slot-scope="{row}">
              <span :style="{ color: setAlarmStatusColor(row.status) }">{{ setAlarmStatus(row.status) }}</span>
            </template>
          </el-table-column>
          <el-table-column label="操作" align="left" :width="setWidth('操作')" class-name="small-padding fixed-width">
            <template slot-scope="{row}">
              <el-button class="marginR" type="primary" size="mini" @click="handleCheck(row)">查看</el-button>
              <!-- <el-button v-permission="['Close']" type="primary" size="mini" @click="handleClose(row)">关闭</el-button> -->
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  mounted() {
    ...
    console.log('XAies', curretMonthDayCount, time, this.callbackDays)
    // 计算近31天日期
    var date = new Date()
  ...
  methods: {
    ...
    loadAlarmLevel() {
      request({
        ...
        const levelsData = this.localStorageOperation('get', 'levelsList2')
        console.log(levelsData, 'levelsDatalevelsDatalevelsDatalevelsData')
    ...
    handleCatLevelClick(item) {
      ...
      this.localStorageOperation('set', 'levelsList2', levelsList)
      this.findAssetsAlarm()
    },
    // 查看告警弹窗
