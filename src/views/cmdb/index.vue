    ...
    <div class="list-content">
      <div class="table-wrapper">
        ...
        >
          ...
          <el-table-column label="操作" class-name="small-padding fixed-width" width="100" align="center">
            <template slot-scope="{row}">
              <el-button v-permission="['CmdbItemBetweenMap']" type="primary" size="mini" @click="handlerClickItemConfig(row, ['hyHostId', 'hyHostName'])">关系图</el-button>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  methods: {
    ...
    findCmdbItemType() {
      request({
        ...
        if (res.code === 1) {
          ...
          // 默认执行导航栏第一行
          this.$set(this.listQuery, 'itemTypeId', this.localStorageOperation('get', 'HY_ITEM_TYPE_ID') || this.classifyTreeList[0].id)
          this.findCmdbItem()
        }
      })
    },
    ...
    chnClick(val) {
      ...
      this.localStorageOperation('set', 'HY_ITEM_TYPE_ID', val.id)
    },
    handleCreate() {
      this.$router.push({
        name: 'ItemCreate'
