<template>
  <div class="opm-sidebar">
    ...
    <div class="sidebar-box">
      <el-menu background-color="#e5ecf6" :default-active="setDefaultActive()" text-color="#333" active-text-color="#0145ab" @open="expansionChildren($event, true)" @close="expansionChildren($event, false)">
        <el-submenu v-for="(item,index) in classifyList" :key="item.serviceId" :index="index+''">
          <template slot="title">
            <el-tooltip effect="dark" :content="item.serviceName" placement="right">
              <div class="serviceName">
                {{ item.serviceName }}
              </div>
            </el-tooltip>
          </template>
          <el-menu-item v-for="(item2,index2) in item.children" :key="item2.serviceId" :index="`${++index}-${++index2}`" @click="chooseOne(item,item2,index,index2)">
            <div class="serviceMessage">
              <div class="serviceName">
                <el-tooltip effect="dark" :content="item2.serviceName" placement="right">
                  <div class="serviceName">
                    {{ item2.serviceName }}
                  </div>
                </el-tooltip>
...
export default {
  ...
  computed: {
    setStyle() {
      return (serviceId) => {
        return {
          backgroundColor: this.current.serviceId === serviceId ? '#000000 !important' : ''
        }
      }
    },
    selectOrNot() {
  ...
  methods: {
    ...
    // 初始化
    async initData() {
      this.classifyList = await this.findOpsOrderProblemService({ pid: -1, limit: -1, ids: this.serviceIDS, type: 0})// type:0事项
