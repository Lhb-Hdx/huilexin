<template>
  <div class="matter-table">
    <div v-if="itemType === 'todo-approve'" class="todoApprove">
          ...
          <template slot-scope="{ row }">
            <el-dropdown @command="handlerActtion">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <el-dropdown-item v-if="itemType === 'todo-approve'" :command="{ fn: 'handlePush', data: [row] }">查看</el-dropdown-item>
                <el-dropdown-item v-if="itemType === 'todo-approve' && tabsItemCode === 'approve' && row.isApprove" :command="{ fn: 'handlePass', data: [1, row, 1] }">通过</el-dropdown-item>
                <el-dropdown-item v-if="itemType === 'todo-approve' && tabsItemCode === 'approve' && row.isApprove" :command="{ fn: 'handlePass', data: [1, row, 2] }">不通过</el-dropdown-item>
                <el-dropdown-item v-if="itemType === 'todo-approve' && tabsItemCode === 'approve' && row.isApprove" :command="{ fn: 'handleReject', data: [1, row] }">驳回</el-dropdown-item>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
    ...
    <div v-else class="else">
      <el-table :row-class-name="setRowExpandStyle()" :data="getTableDataProp" fit highlight-current-row style="width: 100%" @expand-change="openGroupChildren">
        ...
        <el-table-column prop="updateTime" label="提交时间" align="right" width="auto" min-width="10%" show-overflow-tooltip>
          <template slot-scope="{ row }">
            <div> {{ getCreateTime( row.updateTime || row.createTime) }}</div>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="groupId" label="指定运维小组" align="left" width="auto" min-width="10%" show-overflow-tooltip>
          <template slot-scope="{ row }">
            {{ row.groupName }}
          </template>
        </el-table-column>
          ...
          <template slot-scope="{ row }">
            <el-dropdown @command="handlerActtion">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <!-- 事项流程 && 撤回 && 当前用户为数据添加用户 -->
                <el-dropdown-item v-if="itemType === 'submission' && tabsItemCode ==='rollBack' && +row.userId === +loginUser.userId " :command="{ fn: 'handleEdit', data: [row] }">编辑</el-dropdown-item>
                <el-dropdown-item v-if="itemType === 'todo-handle' && tabsItemCode === 'todoHandle' && +row.userId !== +loginUser.userId" :command="{ fn: 'handleLockStatus', data: [row] }">
                  领取
                </el-dropdown-item>
                <el-dropdown-item v-if="itemType === 'todo-handle' && tabsItemCode === 'todoHandle'" :command="{ fn: 'handleAssigned', data: [row] }">指派</el-dropdown-item>
                <!-- 事项流程 && 待受理  && 当前用户为数据添加用户 -->
                <el-dropdown-item v-if="itemType === 'submission' && tabsItemCode === 'todoHandle' && +row.userId === +loginUser.userId" :command="{ fn: 'handleDelete', data: [row] }">删除</el-dropdown-item>
                <!-- 事项流程 && 待受理 || 待安排 || 处理中 && 当前用户为数据添加用户 -->
                <el-dropdown-item v-if="itemType === 'submission' && (tabsItemCode === 'todoHandle'||tabsItemCode === 'waitPlan'||tabsItemCode === 'handling') && +row.userId === +loginUser.userId" :command="{ fn: 'handleRollBack', data: [row] }">撤回</el-dropdown-item>
                <template
                  v-if="
                    ((itemType === 'todo-handle' && tabsItemCode === 'todoComfirm') || (itemType === 'todo-todo' && tabsItemCode === 'todoComfirm')) &&
                      row.statue === 3 &&
                      row.processState === 5 &&
                      +row.lockUserId === +loginUser.userId
                  "
                >
                  <el-dropdown-item :command="{ fn: 'handleSuccess', data: [row] }"> 成功处理 </el-dropdown-item>
                  <el-dropdown-item :command="{ fn: 'handleFail', data: [row] }"> 处理失败 </el-dropdown-item>
                </template>
              </el-dropdown-menu>
            </el-dropdown>
          </template>
...
export default {
  ...
  methods: {
    ...
    async setTableAboutData(newVal, oldVal) {
      ...
      // 查询任务
      let task = []
      const findOpsOrderWorkTask = await this.findOpsOrderWorkTask({ orderProblemIds: this.$tool.uniqueArray(orderProblemIds).toString(), limit: -1 })
      if (findOpsOrderWorkTask && findOpsOrderWorkTask.data) {
        task = findOpsOrderWorkTask.data
        findOpsOrderWorkTask.data.forEach(item => {
          if (item.userId) userIds.push(item.userId)
        })
      }
      ...
      data.forEach(item => {
        ...
        // 设置任务
        if (task.length > 0) {
          const getTaskIndex = task.findIndex(items => +items.orderProblemId === +item.id)
          if (getTaskIndex !== -1) {
            // 任务指派人
            const getDoTaskUserIndex = user.findIndex(items => +items.userId === +task[getTaskIndex].userId)
            if (getDoTaskUserIndex !== -1) {
              task[getTaskIndex].doTaskUserRealName = user[getDoTaskUserIndex].realName
              task[getTaskIndex].doTaskUserTel = user[getDoTaskUserIndex].tel
            }

            if (!item.children) {
              item.children = [task[getTaskIndex]]
            } else {
              item.children.push(task[getTaskIndex])
            }
          }
        }
    ...
    // 查看
    handlePush(row) {
      this.$router.push({ name: 'matterDetail', query: { id: row.id, alarmId: row.alarmId, groupId: row.groupId } })
    },
    // 事项编辑
    handleEdit(row) {
      this.$router.push({ name: 'matterApplication', query: { id: row.id, alarmId: row.alarmId, groupId: row.groupId} })
    },
    // 点击删除
    handleDelete(row) {
      this.$confirm('确认删除事项吗？', '提示', {
        ...
        // if (row.alarmId) { this.updateZbxAlarmLog(row.alarmId) }
        // 改变故障告警状态为待处理
      })
    },
    handleRollBack(row) {
    ...
    handleSuccess(row) {
      this.$confirm('确认此事项已成功处理吗？', '提示', {
        ...
        // const findOpsOrderWorkTask = await this.$request({ url: this.api.opm.findOpsOrderWorkTask, data: { orderProblemId: row.id, limit: -1 } })
