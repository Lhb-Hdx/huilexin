<template>
  <div>
    ...
    <template v-if="look || (+loginUser.userId === +info.matter.userId)">
      <h3 v-if="analysisList.length>0" class="under-line font-16 mb-10">总结&方案</h3>
      <div class="detailAnalyze-content margin-top-15">
        <div v-for="item in analysisList" :key="item.id" class="detailAnalyze-content-item">
          ...
          <div class="detailAnalyze-content-item-box">
            <div class="box-left">
              <div class="flex space-between" v-html="item.content" />
      ...
      <el-empty v-if="!analysisList.length" description="暂无总结&方案" />
    </template>
...
export default {
  ...
  computed: {
    ...mapState('opm', ['managerId', 'opmTaskList']),
    id() { return this.$route.query.id },
    look() {
      // 作业中的工程师以及主管及锁定人可编辑查看，申请人可查看
      return (this.opmTaskList.some(it => +it.userId === +this.loginUser.userId) || +this.loginUser.userId === +this.managerId || +this.loginUser.userId === +this.info.matter.lockUserId) && ![0].includes(this.info.matter.processState)
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async handleSave() {
      ...
      console.log(insertOpsOrderAnalysisResult)
      // analysisList
    },
    // 新增分析
