  ...
  <div class="my-submission">
    ...
    <div class="rightContainer">
      <div class="tabsAndTable">
        <MatterTabs ref="MatterTabs" item-type="my-apply" :statistics-count-param="statisticsCountParam" :tabs-list="tabsList" @emitSetTableList="emitSetTableList" @emitHandlerSearch="emitHandlerSearch" />
      </div>
      <div class="tableWrapper">
        <MatterTable item-type="my-apply" :table-data="tableData" :table-count="tableCount" @emitGetPagingTableData="emitGetPagingTableData" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    const typeTabs = await this.$store.dispatch('opm/getTypeTabs', {type: 'my-apply'})
  ...
  methods: {
    ...
    //   if (this.currentTab.code === 'reject')
      this.$set(this.listQuery, 'userId', this.$auth.getUserSession().userId)
