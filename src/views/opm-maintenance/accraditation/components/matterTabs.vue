      ...
      <div class="submissionWrapper">
        <div class="stateWrapper">
          ...
          <div class="searchWrapper flex">
            <el-button class="margin-right-5" type="primary" @click="handlerToAdd">流程申请</el-button>
  ...
  methods: {
    ...
    async getTabCount(getGroupIds) {
      for (let i = 0; i < this.statisticsCountParam.length; i++) {
        ...
        if (this.statisticsCountParam[i].findMyKey === 'approveUserId') query['approveUserId'] = this.$auth.getUserSession().userId
        const { code, map } = await this.$request({
          url: this.statisticsCountParam[i].api,
          data: {...query, type: 1, serviceType: this.$parent.currentServiceType}
        })
        ...
        this.getTabProp.forEach(item => {
          ...
          // 审批中|已撤回统计
          if (item.stateField === 'processState') {
            tabCount.forEach(it => {
              if (item.tabQuery['processState'] === it['processState']) {
                item.count = it['processStateCount']
              }
            })
          }
        })
    ...
    handlerToAdd() {
      this.$router.push({ name: 'accraditationApplication' })
    }
  }
}
</script>
