...
import OpmPlanView from '../components/opmPlanView'
import OpmWorkLogView from '../components/opmWorkLogView'
...
import OpmEvaluate from '../components/opmEvaluate'
...
export default {
  ...
  data() {
    return {
      ...
      logState: [
        { id: 1, value: '创建了运维事项,待工程师接单后跟进处理' },
        { id: 2, value: '指派' },
        { id: 3, value: '领取' },
        { id: 4, value: '解锁' },
        { id: 5, value: '关闭' },
  ...
  async created() {
    const newOaActivityList = await this.findNewOaActivity(496)
  ...
  methods: {
      ...
      if (probloemCode !== 1) {
        ...
        if (problemLogs.length !== 0) {
          ...
          problemLogs.forEach(item => {
            ...
            // 记录log
            let approveLog = `${userName} ${this.getLogsValueByState(item.status, receiveName)}`
          ...
          }).filter(it => ![4, 5, 6].includes(it.approveStatue))
        }
      }
    ...
    async getOmRequest() {
      ...
      // 未解锁
      // lockStatue===1锁住
      if (this.info.matter.lockStatue === 1) {
        // 判断锁住事项的用户是否是同一个
        if (Number(this.info.matter.lockUserId) === Number(this.loginUser.userId)) {
          this.components = [
          ...
          // 进入待审核，不应该有个人日志
          if (this.info.matter.statue === 5) {
            const idx = this.components.findIndex(item => item.componentName === 'OpmWorkLogView')
            this.components.splice(idx, 1)
          }
          this.components.forEach((item, index) => {
            // if (this.$auth.getVuexStoragePermission('OpmPlanView') !== -1 && this.info.matter.statue !== 2) {
          ...
          // 计划在待审批的时候才出现
          this.components.forEach((item, index) => {
            if (item.componentName === 'OpmPlanView') {
              if (this.info.matter.processState !== 4 && this.info.matter.statue !== 4) {
                this.components.splice(index, 1)
              }
            }
          })
          ...
          if ([2, 3].includes(this.info.matter.processState)) this.components.push({ label: '评价', componentName: 'opmEvaluate' })
        }
      } else {
        if (Number(this.info.matter.lockUserId) === Number(this.loginUser.userId)) {
          this.components = [{ label: '概况', componentName: 'OpmDetailOverview' }]
        }
      }
    ...
    failMatter() {
      this.$confirm(`确认不通过此审批吗？`, '提示', {
        ...
        if (completeActivity === 1) {
          // const updateData = { id: this.id, statue: 7, processState: 3 }
    ...
    async findTask(logData) {
      ...
      if (taskModelObj.length !== 0) {
        ...
        if (taskModelObj.userId) {
          this.isApprove = taskModelObj.userId.includes('' + this.loginUser.userId)
    ...
    rollBackCommit() {
      this.$confirm(`确认提交审批该事项吗？`, '提示', {
        ...
        .then(async () => {
          ...
          const completeActivity = await this.completeActivity({
            // 5 提交
            taskId: this.filterApprove[0].taskId,
            approveStatue: 5
          })
