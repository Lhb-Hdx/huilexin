...
export default {
  ...
  methods: {
    ...
    async initData() {
      // let opmServiceList = await this.findOpsOrderProblemService({ pid: -1, limit: -1, ids: 97, type: 0 })
      let opmServiceList = await this.findOpsOrderProblemService({ pid: -1, limit: -1, ids: this.serviceIDS, type: 0 })
      ...
      // const opmServiceListChilidren = await this.findOpsOrderProblemService2({ pids: pids, limit: -1, ids: 98 })
      ...
      console.log('编辑时获取到的服务类型ID@', this.matterAppicationProps, serviceParentId, serviceChildId)
      if (serviceParentId) {
        ...
        console.log('左侧服务类型数据@2', selectParentIndex, selectChildIndex, this.opmServiceList, childernData)
      }
