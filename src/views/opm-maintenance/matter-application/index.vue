<template>
  <div class="app-container createMatter">
          ...
          <el-row :gutter="15">
            <el-col :span="14">
              <el-form ref="ruleForm" label-position="left" :model="rulesForm" :rules="rules" label-width="120px">
                <el-form-item label="服务类型:" prop="problemType">
                  {{ sidebarMessage.childName ? `${sidebarMessage.pName} > ${sidebarMessage.childName}` : '' }}
                </el-form-item>
                ...
                <CustomVFormRender ref="customVFormRender" :form-id="sidebarMessage.formId" :order-id="queryId" />
            ...
            <el-col :span="10">
              ...
              <el-card shadow="never" class="mt-16">
                ...
                />
                <div class="card-button-ground pointer down-template" @click="onDocumentTemplateDownLoad">
                  选择模板下载
              ...
              <el-card shadow="never" class="mt-16">
                ...
                <el-select v-model="HrUserGroupForm.groupId" style="margin-bottom: 12px" @change="showMember">
                  <el-option
                ...
                <div>
                  <el-button id="btn_margin" type="primary" size="small" round>全部成员 {{ userList.length || '' }}</el-button>
                  <el-button v-for="(item, index) in userList" id="btn_margin" :key="index" size="small" round>{{ item.realName }}</el-button>
                </div>
              </el-card>
    ...
    <el-drawer title="申请事项文档模板下载" :visible.sync="dialogDocumentTemplateDrawer" direction="rtl">
...
export default {
  ...
  watch: {
    // 'HrUserGroupForm.groupId': {
    //   handler(newVal, oldVal) {
    //     if (newVal !== oldVal) {
    //       // this.deleteOpsOrderProblemGroupMap(this.queryId, oldVal)
    //     }
    //   }
    // }
  },
  ...
  methods: {
    ...
    async getOpsOrderProblem(id) {
      const {code, model } = await request({url: api.opm.getOpsOrderProblem, data: { id } })
      if (code === 1) {
        return model
      }
    },
    ...
    getMatter() {
      if (this.$route.query.id) {
        ...
        console.log('getOpsOrderProblem@事项列表', this.matterAppicationProps, this.subTypes, this.selectSubTypes)
      }
    },
    ...
    handleSubmit() {
      ...
      this.$refs.ruleForm.validate(async val => {
        const validateVFormInputData = await this.$refs.customVFormRender.validateForm()
        if (validateVFormInputData.validate) {
          if (!val) return
          // this.sumbitFlag = true
          delete this.rulesForm.createTime
          delete this.rulesForm.updateTime
          // 设置事项编号|提交时什么状态,撤回后再编辑也是什么状态 processState
          const processState = this.queryId ? (this.$route.query?.processState ? this.$route.query.processState : 0) : 0
          console.log('撤回后再编辑也是什么状态@', processState)
          ...
          let { model } = await request({
            ...
            data: { ...this.rulesForm, statue: 0, processState }
          })
          ...
          if (!this.queryId) {
            ...
            // 如果服务类型是直接走流程|flow:1
            console.log(' 如果服务类型是直接走流程|flow:1@', this.sidebarMessage.flow)
            if (this.sidebarMessage.flow) {
              const findHrUserGroupMap = await request({
                url: api.sys.findHrUserGroupMap,
                data: {
                  groupId: this.sidebarMessage.groupId || this.HrUserGroupForm.groupId,
                  classify: 'manager'
                }
              })

              if (findHrUserGroupMap.code === 1 && findHrUserGroupMap.data && findHrUserGroupMap.data.length > 0) {
                const data = {
                  id: this.dataId,
                  lockStatue: 1,
                  lockUserId: findHrUserGroupMap.data[0].userId,
                  statue: 1,
                  processState: 4
                }
                const updateOpsOrderProblem = await request({ url: api.opm.updateOpsOrderProblem, data })
                if (updateOpsOrderProblem.code === 1) {
                  await request({
                    url: api.opm.insertOpsOrderProblemLog,
                    data: {
                      ...
                      orderId: this.dataId,
                      status: 3,
                      createTime: dateFormat('YYYY-mm-dd HH:MM:SS', new Date(date.getTime() + 1000))
                    }
                  })
                  // 直接走流程直接通知主管
                  const manager = this.userList.find(it => it.managerOrenginner === 'manager')
                  if (manager) await messageSendToPlatform(4, {receiverIds: manager.userId, title: model.name, problemNo: model.problemNo, status: 4})
                }
              }
            } else {
              // 不是直接走流程通知所有群组人员
              await messageSendToPlatform(1, {receiverIds: this.userList.map(it => it.userId).filter(Boolean).toString(), title: model.name, problemNo: model.problemNo, status: 0})
            }
          } else {
            console.log('编辑事项@', this.queryId)
            const model = await this.getOpsOrderProblem(this.queryId)
            // 如果服务类型是直接走流程|flow:1
            if (this.sidebarMessage.flow) {
              const findHrUserGroupMap = await request({
                url: api.sys.findHrUserGroupMap,
          ...
          // 更新运维小组
          if (this?.oldHrUserGroupFormGroupId.length > 1) {
            const diffGroupId = this.oldHrUserGroupFormGroupId
            const oldGorupId = diffGroupId[0]
            const newGorupId = diffGroupId.at(-1)
            if (newGorupId !== oldGorupId) {
              this.deleteOpsOrderProblemGroupMap(this.queryId, oldGorupId)
              console.log('服务类型组id发生变化', newGorupId, oldGorupId)
            }
          }
    ...
    insertOpsOrderProblemGroupMap(Id, groupId) {
      request({ url: api.opm.insertOpsOrderProblemGroupMap, data: { problemId: Id, groupId } })
    },
    // 切换时删除原来的小组
    deleteOpsOrderProblemGroupMap(Id, groupId) {
      if (Id && groupId) {
        request({ url: api.opm.deleteOpsOrderProblemGroupMap, data: { problemId: Id, groupId } })
      }
      return
    },
    // 提交新增、编辑提交附件
    ...
    setNoByProblemType() {
      ...
      this.$set(this.rulesForm, 'problemNo', `${this.sidebarMessage.problemNo}${Y}${M}${D}${h}${m}${s}${this.userInfo.userId ? this.userInfo.userId : ''}`)
      // }
    },
    // 查找运维小组
    ...
    async initFindSysUser(ids = []) {
      ...
      const { data: userList, code } = await this.$request({ url: this.api.sys.findSysUser, data: form })
      if (code !== -1) {
    ...
    async showMember(item) {
      ...
      if (item) this.oldHrUserGroupFormGroupId.unshift(+item)
      ...
      if (hrUserGroup.length > 0) {
        ...
        if (ids.length > 0) {
          ...
          if (code !== -1) {
            userList.forEach(it => {
              const idx = hrUserGroup.find(item => +item.userId === +it.userId)
    ...
    emitParentService(val) {
      ...
      console.log('默认事项子类型名称@', val, val.childName, isEdit, this.HrUserGroupList)
      if (val.groupId) {
        isEdit ? this.showMember(this.$route.query.groupId) : this.showMember(val.groupId)
      } else {
