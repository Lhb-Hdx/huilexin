  ...
  <div class="todo-approve">
    ...
    <div class="rightContainer">
      ...
      <div class="tableWrapper">
        <MatterTable item-type="todo-approve" :table-data="tableData" :table-count="tableCount" @emitGetPagingTableData="emitGetPagingTableData" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    const typeTabs = await this.$store.dispatch('opm/getTypeTabs', { type: 'todo-approve' })
  ...
  methods: {
    ...
    async generateData() {
      let tableData = []
      let tableCount = 0
      if (+this.menuQuery.tableDataType === 2) this.$set(this.listQuery, `${this.currentTab.findMyKey}`, this.$auth.getUserSession().userId)
      ...
      if (tableData.length > 0) {
        ...
        if (serviceList.length) {
          ...
          const findOpsOrderProblem = await this.findOpsOrderProblem({ ids: this.$tool.uniqueArray(approveIds).toString(), limit: -1, serviceTypes})
