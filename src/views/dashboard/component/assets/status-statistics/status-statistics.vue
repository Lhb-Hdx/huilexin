    ...
    <div class="box-item-content content-height-300">
      ...
      <div class="item-content-body flexBox align-item-center">
        <div class="echart-box">
          <div id="statusStatisticsEcharts" ref="statusStatisticsEcharts" style="height: 220px" />
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  data() {
    return {
      ...
      modelDefine: 'assetsStatusStatistics',
      watchItemShow: null,
      dashboardType: 4,
      modelShow: '',
      devStatus: [
        { value: 0, name: '使用中', color: '#ea605c' },
        { value: 0, name: '闲置中', color: '#ea8735' },
        { value: 0, name: '已报废', color: '#4da88b' },
        { value: 0, name: '维修中', color: '#9b999c' }
      ]
    }
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    createEcharts() {
      const self = this
      const opsChartsObj = echarts.init(self.$refs['statusStatisticsEcharts'])
      const option = {
