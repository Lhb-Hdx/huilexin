<template>
  <div ref="playSliderBox" class="play-slider-box" @mousedown="playBarMoveDown" @mouseup="playBarMouseUp" @mousemove="getPosition($event)">
    ...
    <!--有播放视频的进度条-->
    <div class="play-slider-block" :style="{left : barX + 'px'}" @mousedown="playIngMouseDown" @mouseup="playIngMouseUp" @mousemove="playIngMouseMove" /><!--播放控制条-->
...
export default {
  ...
  watch: {
    ...
    updatePlayerSliderBlockInfo: {
      handler(newVal) {
        if (newVal.left || newVal.left === 0) {
          this.barX = newVal.left
        }
      },
      deep: true
    }
  ...
  methods: {
    ...
    drawSourcesBar() {
      if (this.sources && this.sources.length > 0) {
        ...
        const nowStartDate = this.currentDate ? new Date(`${this.currentDate} 00:00:00`) : new Date(`${nowDate.getFullYear()}-${nowDate.getMonth() + 1}-${nowDate.getDate()} 00:00:00`)
        const nowTime = nowStartDate.getTime() / 1000
        let dayFirst = 0
        if (this.sources[0].stime < nowTime) {
          this.$set(this.sources[0], 'stime', nowTime)
          dayFirst = nowTime
        } else {
          dayFirst = nowTime
        }
    ...
    currentChangePlayTime() {
      ...
      if (blockIdx !== -1) {
        ...
        this.$emit('currentChangePlayTime', { ...this.sources[blockIdx], index: blockIdx, currentBlockSeconds, sliderType: 'video' })
      }
    },
