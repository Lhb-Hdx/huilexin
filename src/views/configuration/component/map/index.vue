<template>
  <div ref="appContainer" class="config-content-main">
    <div
      ...
      class="config-content-main flex-v-center"
    >
      <div id="topoMap" style="width: 1000px;height:'600px'" />
  ...
  methods: {
    ...
    async initMap() {
      ...
      this.topoGraph.on('node:mouseenter', async(evt) => {
        ...
        if (evt.item._cfg.model.userDefineType === 4 && device.hyDevTypeValue === 1000) {
        ...
        that.showLabel = true
        that.labelName = evt.item._cfg.model.label
        that.labelLeft = evt.canvasX - 80
        that.labelTop = evt.canvasY - 80
      })
    },
