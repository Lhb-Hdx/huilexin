  ...
  <div class="wrp">
    ...
    <div class="bodyBox">
      <el-table :data="data" stripe border style="width: 100%">
        ...
        <el-table-column prop="parentApplicationName" label="关联"><template slot-scope="scope">
          <div>{{ scope.row.parentApplicationName }}</div>
        </template></el-table-column>
      ...
      <div class="drawer-wrp">
        <el-form label-position="left" class="demo-table-expand">
          ...
          <el-form-item label="关联公众号">
            <el-select v-model="model.parentId" class="filter-item" placeholder="选择关联公众号">
              <el-option v-for="item in weixinData" :key="item.applicationId" :label="item.applicationName" :value="item.applicationId" />
            </el-select>
          </el-form-item>
          <el-form-item label="">
...
export default {
  ...
  data () {
    return {
      classify: 'wxapp', keyword: null, page: 1, limit: 10, pageSize: [10, 20, 30, 40, 50], weixinData: [], data: [], loading: false, total: 0, editorDrawer: false, model: { applicationId: null, applicationName: null, appKey: null, secret: null, status: 'enable', classify: this.classify }
    }
  },
  mounted () {
    this.loadWeiXinData()
  ...
  methods: {
    loadWeiXinData () {
      var that = this
      request({
        url: '/admin/findSysMobileApplication', data: { limit: -1, classifys: 'weixin', status: 'enable' }
      }).then(res => {
        res.data.forEach(item => {
          that.weixinData.push(item)
        })
      }).finally(() => {
        this.loading = false
      })
    ...
    loadData (obj) {
      ...
      request({
        ...
        res.data.forEach(item => {
          item.parentName = ''
          item.parentApplicationName = null
          item.details = []
          that.data.push(item)
        ...
        if (parentIds.length > 0) {
          request({
            url: '/admin/findSysMobileApplication', data: { limit: -1, classifys: 'weixin', ids: parentIds.join(',') }
          }).then(res2 => {
            if (res2.data) {
              that.data.forEach(item => {
                res2.data.forEach(item2 => {
                  if (item.parentId === item2.applicationId) {
                    item.parentApplicationName = item2.applicationName
                  }
                })
              })
            }
    ...
    onAdd () {
      this.model = { applicationId: 0, applicationName: null, appKey: null, secret: null, status: 'enable', classify: this.classify }
