  ...
  <div class="wrp">
    ...
    <div class="bodyBox">
      <el-table :data="data" stripe border style="width: 100%">
        ...
        <el-table-column prop="crorpid" label="企业ID"><template slot-scope="scope">
          <div>{{ scope.row.crorpid }}</div>
        </template></el-table-column>
        <el-table-column prop="corpsecret" label="密钥"><template slot-scope="scope">
          <div>{{ scope.row.corpsecret }}</div>
        </template></el-table-column>
          ...
          <template slot-scope="props">
            <el-dropdown size="mini" type="primary" @command="handleCommand($event,props.row)">
              ...
              <el-dropdown-menu slot="dropdown">
                ...
                <!-- <el-dropdown-item v-if="props.row.status === 'enable'" command="synchronization">同步用户</el-dropdown-item> -->
              </el-dropdown-menu>
            </el-dropdown>
          </template>
        </el-table-column>
      ...
      <div class="drawer-wrp">
        <el-form label-position="left" class="demo-table-expand">
          ...
          <el-form-item label="密钥">
            <el-input v-model="model.corpsecret" type="text" />
          </el-form-item>
          <el-form-item label="">
...
export default {
  ...
  data () {
    return {
      ...
      model: { applicationId: null, applicationName: null, agentId: null, crorpid: null, corpsecret: null, status: 'enable', classify: this.classify }
    }
  },
  mounted () {
    this.loadData()
  ...
  methods: {
    ...
    onAdd () {
      this.model = { applicationId: 0, applicationName: null, agentId: null, crorpid: null, corpsecret: null, status: 'enable', classify: this.classify }
    ...
    handleCommand (command, item) {
      ...
      //   this.onSynchronizationQyWx(item)//因为对应app_id与company_id需要对应，这里没有，所以要删除
      // }
    },
