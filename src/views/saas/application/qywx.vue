  ...
  <div class="wrp">
    ...
    <div class="bodyBox">
      <el-table :data="data" stripe border style="width: 100%">
        ...
        <el-table-column prop="corpsecret" label="企业微信应用密钥"><template slot-scope="scope"><div>{{ scope.row.corpsecret }}</div></template></el-table-column>
        <el-table-column prop="corpToken" label="通信TOKEN"><template slot-scope="scope"><div>{{ scope.row.corpToken }}</div></template></el-table-column>
        <el-table-column prop="corpEncodingAesKey" label="通信解密KEY"><template slot-scope="scope"><div>{{ scope.row.corpEncodingAesKey }}</div></template></el-table-column>
        <el-table-column prop="suiteId" label="第三方应用ID"><template slot-scope="scope"><div>{{ scope.row.suiteId }}</div></template></el-table-column>
        <el-table-column prop="suiteSecret" label="第三方应用密钥"><template slot-scope="scope"><div>{{ scope.row.suiteSecret }}</div></template></el-table-column>
        <el-table-column prop="providerSecret" label="企业密钥"><template slot-scope="scope"><div>{{ scope.row.providerSecret }}</div></template></el-table-column>
...
export default {
  ...
  methods: {
    loadData(obj) {
      ...
      var that = this
      request({
        url: '/admin/findSysMobileApplication', data: { page: this.page, limit: -1, keyword: this.keyword, classifys: 'qywx,qywx.parent,qywx.self' }
      }).then(res => {
