    ...
    <div class="main">
      ...
      <div class="mbody">
        <div class="items short ">
          <div class="item"><div class="lbl">模板名称</div><div class="val"><el-input v-model="model.templateName" type="text" placeholder="模板名称" /></div></div>
        ...
        <div v-for="(item,index) in data" :key="index" class="items short ">
          ...
          <div class="template">
            ...
            <div v-if="item.editor === true" class="txt"><textarea v-model="item.content" /><i class="el-icon-check" @click="onEditorTemplateSave(item)" /></div>
          </div>
...
export default {
  ...
  data() {
    return {
      keyword: null, page: 1, limit: 10, pageSize: [10, 20, 30, 40, 50], data: [], loading: false, total: 0, classify: 'qywx.self', appData: [], companyData: [],
