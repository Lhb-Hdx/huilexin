    ...
    <div class="drawer-alarm-dialog">
      <div ref="leftRegion" class="left-region">
        ...
        <div class="switch-box">
          ...
          <!-- <div class="show-box" @scroll="handleScroll($event)">  handleScroll触底加载-->
          <div class="show-box">
            <!-- 处理记录 -->
            ...
            <div v-show="showSwitch === 2" v-permission="['SameAlarm']" class="fault-info">
              <template v-if="sameFault.length > 0">
                ...
                <pagination
                  ...
                  :limit.sync="limit"
                  :auto-scroll="false"
                  @pagination="findZbxAlarmLogSameType"
                />
              </template>
              <el-empty v-else description="暂无数据" />
            </div>
...
export default {
  components: {
    DetailedInformation, EquipmentChange, RelevantKnowledge, WiringTable, MaskingAlarmPushDialog, Pagination
  },
  props: {
    model: {
      type: Object,
  ...
  methods: {
    ...
    async handleCheck(data) {
      ...
      await this.findZbxAlarmLogSameType()
      // 设备变更
    ...
    async findZbxAlarmLogSameType() {
      ...
      const query = { keyword: this.getDetail.description, hostid: this.getDetail.hostId, page: this.page, limit: this.count, startCLock: time1, endClock: this.getDetail.clock, agents: this.agentsIds }
      if (this.getDetail.sameType) query.sameType = this.getDetail.sameType
