  ...
  >
    ...
    <div ref="systemInfoRef">
      <el-row v-if="!showEmpty">
        <el-col v-for="(item,index) in databaseLog" :key="index" :span="8">
              ...
              <div class="info-header">
                ...
                <div class="word-data">
                  <div class="word-title">
                    <span>{{ item.name }} 日志文件</span>
                    <span>{{ conversionUnit(item.totalSize,item.totalUnits) }}</span>
                  </div>
                  <div class="progress  no-progress-bar-innerText">
                    <el-progress :text-inside="true" :stroke-width="10" :percentage="Number(item.usePercentageSize)" />
                  </div>
                  <div class="word-size">
                    <span>已用大小：{{ conversionUnit(item.useSize,item.useSizeUnits) }}</span>
                    <span>已用：{{ item.usePercentageSize }}{{ item.usePercentageUnits }}</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  data() {
    return {
      ...
      watchItemKey: 'SQLServerDatabaseLog',
      watchItemShow: null,
      dataList: {},
      showEmpty: false
    }
  ...
  computed: {
    ...
    conversionUnit() {
      return function(val, unit) {
        const data = this.$tool.conversionData('data', val, unit)
        return data.size + data.units
      }
    }
  },
  watch: {
