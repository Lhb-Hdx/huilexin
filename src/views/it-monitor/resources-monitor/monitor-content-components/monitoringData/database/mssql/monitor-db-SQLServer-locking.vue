<template>
  <div v-if="isConfig || (!isConfig && !watchItemShow)" ref="chuckBox" class="chuck-box">
    ...
    >
      ...
      <UsedStatisticsContainer v-if="!showEmpty" echart-el="lockingChart" :echart-data="echartData" :table-data="tableData" :custom-configuration="customConfiguration" />
...
export default {
    ...
    locking: {
      handler(newVal) {
        if (newVal) {
          ...
          this.$set(this.echartData[1], 'itemId', newVal.TimeLockingItemId)
          this.$set(this.echartData[1], 'unit', '')

          // 统计图
          if (newVal.lockingValue) {
            this.$set(this.echartData[0].data, this.echartData[0].data.length, newVal.lockingValue[newVal.lockingValue.length - 1])
            this.$set(this.echartData[0].units, this.echartData[0].units.length, newVal.lockingltemUnits)
          }
          if (newVal.TimeLockingValue) {
            this.$set(this.echartData[1].data, this.echartData[1].data.length, newVal.TimeLockingValue[newVal.TimeLockingValue.length - 1])
            this.$set(this.echartData[1].units, this.echartData[1].units.length, newVal.TimeLockingItemUnits)
          }
          ...
          // 表格
          this.$set(this.tableData[0].col[0], 'value', newVal.lockingValue[newVal.lockingValue.length - 1])
          this.$set(this.tableData[0].col[0], 'unit', newVal.lockingltemUnits)
          this.$set(this.tableData[0].col[1], 'value', newVal.TimeLockingValue[newVal.TimeLockingValue.length - 1])
          this.$set(this.tableData[0].col[1], 'unit', newVal.TimeLockingItemUnits)
        }
      },
      deep: true
    }
  ...
  methods: {
    openUsedDetailDialog() {
      ...
      this.detailParams = [
        {
          itemid: this.locking.queryLockItemId,
          valueType: this.locking.lockingValueType,
          name: '每秒锁定请求总数',
          color: '#87DEAF',
          unit: '',
          echartType: 'value',
        ...
        {
          itemid: this.locking.TimeLockingItemId,
          valueType: this.locking.TimeLockingValueType,
          name: '锁定等待时间',
          color: '#3ba9ed',
          unit: '',
      ...
      this.tableParams = [
        ...
        {
          label: ['最高锁定等待时间', '平均锁定等待时间', '最低锁定等待时间'],
          value: 0,
          unit: '',
          itemid: this.locking.TimeLockingItemId,
          valueType: this.locking.TimeLockingValueType,
