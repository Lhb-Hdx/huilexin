<template>
  <div v-if="isConfig || (!isConfig && !watchItemShow)" ref="monitorVirtualMemory" class="chuck-box">
    <div
      v-loading="powerStatus.length < 1 && !showEmpty"
      element-loading-spinner="el-icon-loading"
      class="chuck-box-content box-border margin-left-7-5 margin-right-7-5 margin-top-15"
    >
      <div class="head-bar head">
        <div class="title">电源状态</div>
      ...
      <div class="device-list">
        <el-empty v-if="powerStatus.length === 0 || showEmpty" :image-size="100" description="暂无数据" />
        <template v-else>
          <div v-for="item in powerStatus" :key="item.itemid" class="device-item" @mousemove="mousemoveBoardCard($event, item)" @mouseleave="mouseleaveBoardCard()">
            <div class="device-item-header">
              <div class="item-header-icon">
                ...
                <img src="image/it-monitor/ico_dy.png" alt="">
              </div>
...
export default {
  ...
  mounted() {
    ...
    const that = this
    setTimeout(() => {
      if (that.powerStatus.length < 1) that.showEmpty = true
    }, 10000)
  },
  methods: {
