...
export default {
  ...
  methods: {
    clickCatalogue() {
      this.$EventBus.$emit('click-catalogue')
    },
    // 添加分组
    ...
    async deleteHyitOpsScriptGroup(query) {
      const { code } = await this.$request({
        url: this.api.diagram.deleteDiagramCabinetGroup,
        data: query
      })
      if (code !== 1) return
      return code
    ...
    async getHyitOpsScriptGroup(query) {
      const { code, model } = await this.$request({
        url: this.api.diagram.getDiagramCabinetGroup,
        data: query
      })
      if (code !== 1) return
      return model
    ...
    confirmGroup() {
      ...
      this.$refs['form'].validate(valid => {
        if (valid) {
          const requestTopologyInfoApi = this.formId ? this.api.diagram.updateDiagramCabinetGroup : this.api.diagram.insertDiagramCabinetGroup
          if (this.formId) {
            data.id = this.formId
          }
          this.$request({
