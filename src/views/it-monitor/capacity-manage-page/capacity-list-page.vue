<template>
  <div class="app-container flex" style="padding:0">
    <div class="page-left sideMenu" :style="{height:clientHeight + 'px'}">
      <div class="tree-header flex space-between align-item-center">
        <div class="tree-header-title pointer" @click="clickCatalogue">目录</div>
      </div>
      <div class="menu-container" @scroll="orgTreeScroll">
        <CapacityAreaTree :org-tree-list="orgTreeList" @click-directory="clickdirectory" @click-detail="onDetail" />
      </div>
    </div>
      ...
      <template v-if="!handlerCabinetData.cabinetId">
        <div class="app-container">
          <div class="filter-container">
            ...
            <el-button class="filter-item" type="primary" @click="create">新增</el-button>
            <el-input v-model="listQuery.keyword" placeholder="搜索关键词" class="filter-item" clearable style="width: 200px" />
          ...
          <div class="table-wrapper">
            <el-table :data="cabinetList">
              <el-table-column v-if="lists.length > 0 ? lists[0].ispass : true" align="left" label="机柜名称" prop="name" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[1].ispass : true" align="right" label="U位" prop="capacity" width="100px" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[2].ispass : true" align="right" label="已用" prop="usedCount" width="100px" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[3].ispass : true" align="right" label="可用" prop="availableCount" width="100px" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[4].ispass : true" align="left" label="可用度" width="200px">
                <template slot-scope="{row}">
                  <el-progress :percentage="setAvailability(row)" />
                </template>
              </el-table-column>
              <el-table-column v-if="lists.length > 0 ? lists[5].ispass : true" align="right" label="额定供电（KW）" prop="ratedPower" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[6].ispass : true" align="right" label="重量（KG）" prop="weight" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[7].ispass : true" align="left" label="描述" width="220" prop="des" show-overflow-tooltip />
              <el-table-column v-if="lists.length > 0 ? lists[8].ispass : true" align="left" label="操作" width="300px">
                <template slot-scope="{row}">
                  ...
                  <el-button type="primary" size="mini" @click="onConfig(row)">配置</el-button>
        ...
        <el-form ref="form" :model="form" :rules="rules" label-width="110px">
          ...
          <el-form-item label="分组" prop="groupId">
            <!-- <AiTreeInput
              :key="reloadKey"
              :level-start-num="0"
              :field="{'id':'hyAreaId','name':'hyAreaName','parent':'hyPid'}"
              :remote="api.area.findHyitArea"
              :infoapi="api.area.getHyitArea"
              placeholder="请输入上层分组"
              :param="{limit: -1}"
...
import CapacityAreaTree from './components/capacity-area-tree'
...
export default {
  ...
  data() {
    ...
    return {
      ...
      orgTreeList: [],
      orgList: [],
      orgProps: { label: 'hyAreaName', value: 'hyAreaId', checkStrictly: true, emitPath: false },
      clientHeight: (document.body.clientHeight - 80),
      // 显示控制数据 start
      checkList: ['机柜名称', 'U位', '已用', '可用', '可用度', '额定供电（KW）', '重量（KG）', '描述', '操作'],
      lists: []
      // 显示控制数据 end
    }
  },
  ...
  async mounted() {
    ...
    window.onresize = function temp() {
      _this.clientHeight = document.body.clientHeight - 80
    }
    this.findGroupData()
    this.getOrgTree()
  ...
  methods: {
    ...
    getOrgTree() { // 读取组织结构信息
      const that = this
      this.$request({
        url: this.api.area.findHyitArea, data: { limit: -1 }
      }).then(async res => {
        const num = this.$tool.listToTree(res.data, 0, 0, 'hyAreaId', 'hyPid')
        this.orgList = this.mapTree(num)
        const val = await that.findDiagramCabinetData()
        console.log(val)
        res.data.forEach(items => {
          items.active = true
          items.dvr = []
          val.forEach(item => {
            if (items.hyAreaId === item.groupId) {
              items.dvr.push(item)
            }
          })
        })
        const data = this.$tool.listToTree(res.data, 0, 0, 'hyAreaId', 'hyPid')
        this.orgTreeList = this.mapTree(data)
        console.log(this.orgTreeList)
      })
    },
    // 查询机柜
    async findDiagramCabinetData() {
      ...
      res.data.forEach(item => {
        item.hyAreaId = item.cabinetId
        item.hyPid = item.groupId
      })
      return res.data
    },
