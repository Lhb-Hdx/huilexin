<template>
  <div ref="statisticsAlarmTop" class="statistics-alarm-top" :style="{ 'height': paneHeight + 'px' }">
...
export default {
  ...
  methods: {
    ...
    async getHostAlarmLogCount() {
      // 查询top5主机
      const getHostAlarmLogCount = await this.$request({ url: this.api.itMonitor.getHostAlarmLogCount, data: { page: 1, limit: 5, hostIds: this.$tool.uniqueArray(this.hostIds).toString(), ...this.clockQuery }})
      ...
      const topHostAlarm = []
      top.forEach(item => {
        const hostIndex = findHyitHost.data.findIndex(el => +el.hyHostId === +item.hostId)
        if (hostIndex !== -1) {
          let tmp = {
          ...
          if (hostIndex !== -1) {
            tmp = { ...tmp, ...findHyitHost.data[hostIndex] }
          }
          topHostAlarm.push(tmp)
        }
      })
      this.list = topHostAlarm
