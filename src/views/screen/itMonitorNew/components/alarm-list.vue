...
export default {
  ...
  watch: {
    alarmCountHeight: {
      async handler() {
        this.alarmListHeight = await this.$store.dispatch('screen/getAlarmHeightSize')
      }
    }
  },
  mounted() {
  ...
  beforeDestroy() {
    this.batchOffEventBus('handler-top-project,start-up-date,load-alarm-list')
  ...
  methods: {
    ...
    async init() {
      ...
      this.alarmList = []
      if (this.agents && this.agents.length > 0) await this.getAlarmData()
    },
    // 设置相关尺寸
    async setSize() {
      this.alarmListHeight = await this.$store.dispatch('screen/getAlarmHeightSize')
