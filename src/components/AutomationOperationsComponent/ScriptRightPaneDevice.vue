<template>
  <div class="device-form">
    <div class="device-form-item">
      <div class="form-item-label">选择设备</div>
      <div class="form-item-main">
        <el-button size="mini" type="primary" icon="el-icon-plus" @click="openSelectionWindow">添加设备</el-button>
        <el-table :data="allSelection" border fit highlight-current-row style="width: 100%;margin-top: 10px">
          <el-table-column label="服务器名称" prop="hyMarkName" min-width="150" show-overflow-tooltip />
          <el-table-column label="IP" prop="hyIp" min-width="100" show-overflow-tooltip />
          <el-table-column label="状态" prop="hyMarkName" min-width="50" show-overflow-tooltip>
            <template slot-scope="{row}">
              <div :class="row.hyNetState === 1 ? 'success' : 'danger'">{{ row.hyNetState === 1 ? '在线' : '离线' }}</div>
            </template>
          </el-table-column>
          <el-table-column label="操作" width="100">
            <template slot-scope="{row}">
              <el-button type="danger" size="mini" @click="deleteSelectDevice(row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <el-button class="margin-top-10" size="mini" type="primary" @click="implement">执行脚本</el-button>
      </div>
    </div>
