    ...
    <transition-group class="transition-wrapper" name="sort">
      ...
      >
        ...
        <div v-if="groupItem.expand" class="tree-children">
          <!-- 子级分组 -->
          <script-group-tree v-if="groupItem.group.length > 0" :tree-list="groupItem.group" />
        </div>
      </div>
    </transition-group>
  </div>
...
export default {
  ...
  beforeDestroy() {
    this.batchOffEventBus('add-group,delete-group,update-group,handler-tree-item')
  },
  mounted() {
    this.getTreeList = this.$tool.deepCopy(this.treeList)
  ...
  methods: {
    ...
    async handlerTreeItem(val) {
      this.$EventBus.$emit('handler-ops-group-item', this.$tool.deepCopy(val))
    ...
    dragend(value, e) {
      ...
      if (this.oldData !== this.newData) {
        ...
        this.$request({
          url: this.api.automationOperations.updateHyitOpsScriptGroupIndex,
          data: {
